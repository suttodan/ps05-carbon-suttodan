...
countries <- demog %>%
  ...
  summarize(iso3Total = n_distinct(iso3),
            iso2Total = n_distinct(iso2),
            countryTotal = n_distinct(name))

countries %>% 
  knitr::kable()
```
There are `r countries$countryTotal` countries in the dataset. It appears there are `r countries$iso2Total` iso-2 codes, and `r countries$iso3Total` iso-3 codes. However, when filtering for only combinations that include "NA" for an iso-2 codes, we see that there are two possibilities for name: an accompanying "NA" value, or Namibia. 
